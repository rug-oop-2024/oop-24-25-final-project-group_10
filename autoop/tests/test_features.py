...
class TestFeatures(unittest.TestCase):
    ...
    def test_detect_features_continuous(self):
        """
        Test detecting features with continuous data.
        """
        iris = load_iris()
        df = pd.DataFrame(
            iris.data,
    ...
    def test_detect_features_with_categories(self):
        """
        Test detecting features with categorical data.
        """
        ...
        for detected_feature in filter(lambda
                                       x: x.name in numerical_columns,
                                       features):
            self.assertEqual(detected_feature.type, "numerical")
        for detected_feature in filter(lambda
                                       x: x.name in categorical_columns,
                                       features):
